...
class EmailTest {
    ...
    @DisplayName("로컬 파트 최대 길이와 동일한 이메일이 주어졌을 때 이메일 생성에 성공해야한다.")
    @Test
    void validateSuccessWithLocalPartLength() {
        String email = "a".repeat(Email.EMAIL_LOCAL_PART_MAX_LENGTH) + "@" + "example.com";

        assertThatCode(() -> Email.of(email))
            .doesNotThrowAnyException();
    }

    @DisplayName("도메인 파트 최대 길이와 동일한 이메일이 주어졌을 때 이메일 생성에 성공해야한다.")
    @Test
    void validateSuccessWithDomainPartLength() {
        String email = "username@" + "a".repeat(Email.EMAIL_DOMAIN_PART_MAX_LENGTH);
    ...
    @DisplayName("전체 길이가 정확히 최대 길이인 이메일이 주어졌을 때 이메일 생성에 성공해야한다.")
    @Test
    void validateSuccessTotalLength() {
        String email = "a".repeat(64) + "@" + "b".repeat(Email.EMAIL_DOMAIN_PART_MAX_LENGTH);

        assertThatCode(() -> Email.of(email))
            .doesNotThrowAnyException();
    }

    @DisplayName("도메인 파트에 하이픈(-)과 점(.) 사용한 이메일이 주어졌을 때 이메일 생성에 성공해야한다.")
    @Test
    void validateSuccessAllowedDomain() {
        String email = "user@example-domain.com";
    ...
    void validateFailWithLocalPartLength() {
        String email = "a".repeat(Email.EMAIL_LOCAL_PART_MAX_LENGTH + 1) + "@" + "example.com";
    ...
    void validateFailWithDomainPartLength() {
        String email = "user@" + "a".repeat(Email.EMAIL_DOMAIN_PART_MAX_LENGTH + 1);
