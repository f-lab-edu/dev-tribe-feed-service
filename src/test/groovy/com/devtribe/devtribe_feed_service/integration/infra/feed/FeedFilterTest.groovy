...
import static com.devtribe.devtribe_feed_service.test.utils.fixtures.post.FeedFilterOptionFixture.createFeedFilterOption
...
class FeedFilterTest extends AbstractIntegrationTest {
    ...
    def "키워드가 주어질때 키워드를 포함하는 title을 가진 피드 리스트를 반환한다."() {
        given:
        def filter = createFeedFilterOption(keyword: "백엔드")
        ...
        then:
        result.data().size() == 2
        result.data().collect { it.title } == ["백엔드 로드맵", "백엔드 개발 기술 향상"]
    }

    def "키워드가 주어질때 키워드를 포함하는 content을 가진 피드 리스트를 반환한다"() {
        given:
        def filter = createFeedFilterOption(keyword: "성능 최적화")
        def request = createFeedSearchRequest(filter: filter, sort: FeedSortOption.NEWEST)
